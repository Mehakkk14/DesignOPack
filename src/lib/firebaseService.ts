...
export const addQuoteRequest = async (
  ...
  try {
    ...
    logger.emoji.success(
      "Quote request added successfully with ID:",
      docRef.id,
    );
    return { success: true, id: docRef.id };
  } catch (error) {
...
export const getQuoteRequests = async () => {
  try {
    ...
    const q = query(
      collection(db, quotesCollection),
  ...
  try {
    ...
    const unsubscribe = onSnapshot(
      ...
      (querySnapshot) => {
        ...
        logger.emoji.success(
          "Real-time quote update received:",
          quotes.length,
          "quotes",
        );
        callback(quotes);
      ...
      (error) => {
        logger.emoji.error(
          "Error in quote requests real-time listener:",
          error,
        );
      },
    );
  ...
  try {
    ...
    if (success && banners.length >= 8) {
      ...
      return {
        success: false,
        error:
          "Maximum number of banners (8) reached. Please delete a banner before adding a new one.",
      };
    }
...
export const getActiveBanners = async () => {
  try {
    ...
    if (success) {
      ...
      logger.log(
        `Found ${activeBanners.length} active banners out of ${banners.length} total`,
      );
